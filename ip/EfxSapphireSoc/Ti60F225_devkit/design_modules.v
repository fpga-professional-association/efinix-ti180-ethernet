...
module memctrl_reset_gen #(
    parameter OUT_PULSE_LEN = 4'd3 // no more than 15, because the counter is 4-bit
)(
    output reg o_memctrl_rst,
    input  soc_asyncReset,
    input  soc_systemReset,
    input  soc_memoryReset,
    input  clk
);

localparam S_INIT          = 2'b00;
localparam S_WAIT_ALL_HIGH = 2'b01;
localparam S_WAIT_ALL_LOW  = 2'b11;
localparam S_OUTPUT_PULSE  = 2'b10;

localparam OUTPULSE_CNT_LIMIT = OUT_PULSE_LEN - 'd1 ;

reg [1:0] present_state;
reg [1:0] next_state;
reg [3:0] outpulse_cnt;

always @(posedge clk or posedge soc_asyncReset) begin
    if(soc_asyncReset) begin
...
always @(*) begin
    case(present_state)
        S_INIT: begin
            next_state = S_WAIT_ALL_HIGH;
        end
         
        S_WAIT_ALL_HIGH: begin
            if(soc_systemReset & soc_memoryReset) begin
                next_state = S_WAIT_ALL_LOW;
            end else begin
                next_state = S_WAIT_ALL_HIGH;
            end 
        end

        S_WAIT_ALL_LOW: begin
            if(~soc_systemReset & ~soc_memoryReset) begin
                next_state = S_OUTPUT_PULSE;
            end else begin
                next_state = S_WAIT_ALL_LOW;
            end
        end
        ...
        S_OUTPUT_PULSE: begin
            if(outpulse_cnt < OUTPULSE_CNT_LIMIT) begin
                next_state = S_OUTPUT_PULSE;
            end else begin
                next_state = S_INIT;
            end 
...
always @(posedge clk or posedge soc_asyncReset) begin
    if(soc_asyncReset) begin
        ...
        if(present_state == S_OUTPUT_PULSE) begin
            outpulse_cnt <= outpulse_cnt + 'd1;
            o_memctrl_rst <= 1'b1;
        end else begin
            outpulse_cnt <= 'd0;
            o_memctrl_rst <= 1'b0;
